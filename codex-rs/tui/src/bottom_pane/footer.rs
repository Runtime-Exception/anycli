...
pub(crate) struct FooterProps {
    ...
    /// If Some, shows the current agent mode (e.g., "classic" or "alloy")
...
fn footer_lines(props: &FooterProps) -> Vec<Line<'static>> {
    ...
    match props.mode {
        ...
        FooterMode::ShortcutSummary => {
            ...
            // Show agent mode indicator if available (AnyCLI mode)
            if let Some(ref mode_str) = props.agent_mode {
                line.push_span(" · ".dim());
                line.push_span(Span::from(mode_str.clone()).magenta());
            }
            line.push_span(" · ".dim());
            line.extend(vec![
